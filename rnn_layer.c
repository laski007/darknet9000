...
layer make_rnn_layer(int batch, int inputs, int hidden, int outputs, int steps, ACTIVATION activation, int batch_normalize, int log)
{
    ...
    *(l.output_layer) = make_connected_layer(batch*steps, hidden, outputs, activation, batch_normalize);
    ...
    l.state_gpu = cl_make_array(l.state, batch*hidden*(steps+1));
...
void forward_rnn_layer_gpu(layer l, network net)
{
    ...
    for (i = 0; i < l.steps; ++i) {
        ...
        if(net.train){
			...
			float * sum = state_float + l.hidden*l.batch;
...
void backward_rnn_layer_gpu(layer l, network net)
{
    ...
    increment_layer(&output_layer, l.steps - 1);
	float * state_float = (float *) l.state_gpu;
	float * sum = state_float + l.hidden*l.batch*l.steps;
	l.state_gpu = (cl_mem) sum;
   // l.state_gpu += l.hidden*l.batch*l.steps;
    for (i = l.steps-1; i >= 0; --i) {
      ...
      //  l.state_gpu -= l.hidden*l.batch;
        ...
        s.input_gpu = l.state_gpu;
		float * self_layer_float = (float *) self_layer.delta_gpu;
		s.delta_gpu = self_layer_float - l.hidden*l.batch;
      ...
      //  s.delta_gpu = self_layer.delta_gpu - l.hidden*l.batch;
        ...
        //copy_ongpu(l.hidden*l.batch, self_layer.delta_gpu, 1, input_layer.delta_gpu, 1);
        if (i > 0 && l.shortcut){
			cl_mem new_delta_gpu = (cl_mem) s.delta_gpu;
			axpy_ongpu(l.hidden*l.batch, 1, self_layer.delta_gpu, 1, new_delta_gpu/*self_layer.delta_gpu - l.hidden*l.batch*/, 1);
		}
