...
void train_dcgan(char *cfg, char *weight, char *acfg, char *aweight, int clear, int display, char *train_images)
{
    ...
    float *imerror = cl_make_array(0, y_size);
    ...
    start += 1;
        ...
        for(j = 0; j < gnet.subdivisions; ++j){
            ...
            cl_write_array(gnet.input_gpu, gnet.input, x_size);
            cl_write_array(gnet.truth_gpu, gnet.truth, y_size);
            *gnet.seen += gnet.batch;
            ...
            printf("realness %f\n", cl_checkmag(imerror, imlayer.outputs*imlayer.batch));
            printf("features %f\n", cl_checkmag(gnet.layers[gnet.n-1].delta_gpu, imlayer.outputs*imlayer.batch));
...
void train_colorizer(char *cfg, char *weight, char *acfg, char *aweight, int clear, int display)
{
    ...
    float *imerror = cl_make_array(0, imlayer.outputs*imlayer.batch);
    ...
    while (get_current_batch(net) < net.max_batches) {
        ...
        for(j = 0; j < net.subdivisions; ++j){
            ...
            cl_write_array(net.input_gpu, graypixs, net.inputs*net.batch);
            cl_write_array(net.truth_gpu, pixs, net.truths*net.batch);
            /*
               image origi = float_to_image(net.w, net.h, 3, pixs);
               image grayi = float_to_image(net.w, net.h, 3, graypixs);
               show_image(grayi, "gray");
            ...
            *net.seen += net.batch;
            ...
            printf("realness %f\n", cl_checkmag(imerror, imlayer.outputs*imlayer.batch));
            printf("features %f\n", cl_checkmag(net.layers[net.n-1].delta_gpu, imlayer.outputs*imlayer.batch));
