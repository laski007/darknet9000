...
void forward_region_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
			...
			float * sum_float = output_float + index;
			cl_mem sum = (cl_mem) sum_float;
            activate_array_ongpu(sum/*l.output_gpu + index*/, 2*l.w*l.h, LOGISTIC);
            index = entry_index(l, b, n*l.w*l.h, 4);
			float * sum1_float = output_float + index;
			cl_mem sum1 = (cl_mem) sum1_float;
            activate_array_ongpu(sum1/*l.output_gpu + index*/,   l.w*l.h, LOGISTIC);
        }
    }
    if (l.softmax_tree){
        int i;
        ...
        for (i = 0; i < l.softmax_tree->groups; ++i) {
			...
			float * new_net_float = net.input_gpu + index;
			cl_mem new_net = (cl_mem) new_net_float;
			float * new_output_float = l.output_gpu;
			float * last_para = new_output_float + index;
			cl_mem last = (cl_mem) last_para;			
            softmax_gpu(new_net/*net.input_gpu + index*/, group_size, l.batch*l.n, l.inputs/l.n, l.w*l.h, 1, l.w*l.h, 1, last/*l.output_gpu + index*/);
            count += group_size;
        }
    } else if (l.softmax) {
        int index = entry_index(l, 0, 0, 5);
		...
		float * last_para = new_output_float + index;
		cl_mem last = (cl_mem) last_para;
        softmax_gpu(new_net/*net.input_gpu + index*/, l.classes, l.batch*l.n, l.inputs/l.n, l.w*l.h, 1, l.w*l.h, 1, last/*l.output_gpu + index*/);
    }
    if(!net.train || l.onlyforward){
        cl_read_array(l.output_gpu, l.output, l.batch*l.outputs);
        return;
    }
    ...
    if(net.truth_gpu){
        ...
        cl_read_array(net.truth_gpu, truth_cpu, num_truth);
    }
    cl_read_array(l.output_gpu, net.input, l.batch*l.inputs);
    forward_region_layer(l, net);
    //cuda_push_array(l.output_gpu, l.output, l.batch*l.outputs);
    if(!net.train) return;
    cl_write_array(l.delta_gpu, l.delta, l.batch*l.outputs);
}
...
void backward_region_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
			...
			float * sum_float = delta_float + index;
			cl_mem sum = (cl_mem) sum_float;

			float * output_float = (float *) l.output_gpu;
			float * first_float = output_float + index;
			cl_mem first = (cl_mem) first_float;			
            gradient_array_ongpu(first/*l.output_gpu + index*/, 2*l.w*l.h, LOGISTIC, sum/*l.delta_gpu + index*/);
            index = entry_index(l, b, n*l.w*l.h, 4);
			float * sum2_float = delta_float + index;
			cl_mem sum2 = (cl_mem) sum2_float;

			float * first2_float = output_float + index;
			cl_mem first2 = (cl_mem) first2_float;

            gradient_array_ongpu(first2/*l.output_gpu + index*/,   l.w*l.h, LOGISTIC, sum2/*l.delta_gpu + index*/);
        }
    }
