...
static void increment_layer(layer *l, int steps)
{
	...
	float * sum = output_float + num;
	l->output_gpu = (cl_mem) sum;

	float * delta_float = (float *) l->delta_gpu;
	float * sum1 = delta_float + num;
	l->delta_gpu = (cl_mem) sum1;

//    l->delta_gpu += num;
	float * x_float = (float *) l->x_gpu;
	float * sum2 = x_float + num;
	l->x_gpu = (cl_mem) sum2;
//    l->x_gpu += num;
	float * x_norm_float = (float *) l->x_norm_gpu;
	float * sum3 = x_norm_float + num;
	l->x_norm_gpu = (cl_mem) sum3;
...
layer make_gru_layer(int batch, int inputs, int outputs, int steps, int batch_normalize)
{
    ...
    *(l.state_h_layer) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize);
    ...
    l.forgot_state_gpu = cl_make_array(l.output, batch*outputs);
    l.forgot_delta_gpu = cl_make_array(l.output, batch*outputs);
    l.prev_state_gpu = cl_make_array(l.output, batch*outputs);
    l.state_gpu = cl_make_array(l.output, batch*outputs);
    l.output_gpu = cl_make_array(l.output, batch*outputs*steps);
    l.delta_gpu = cl_make_array(l.delta, batch*outputs*steps);
    l.r_gpu = cl_make_array(l.output_gpu, batch*outputs);
    l.z_gpu = cl_make_array(l.output_gpu, batch*outputs);
    l.h_gpu = cl_make_array(l.output_gpu, batch*outputs);
#endif
...
void forward_gru_layer_gpu(layer l, network net)
{
    ...
    for (i = 0; i < l.steps; ++i) {
		...
		float * output_float = (float *) l.output_gpu;
		float * sum = output_float + l.outputs*l.batch;
		l.output_gpu = (cl_mem) sum;
...
void backward_gru_layer_gpu(layer l, network net)
{
	...
	float * output_float = (float *) l.output_gpu;
	float * sum = output_float + l.outputs*l.batch*(l.steps-1);
	l.output_gpu = (cl_mem) sum;
   // l.output_gpu += l.outputs*l.batch*(l.steps-1);
	float * delta_float = (float *) l.delta_gpu;
	float * sum2 = delta_float + l.outputs*l.batch*(l.steps-1);
	l.delta_gpu = (cl_mem) sum2;
   // l.delta_gpu += l.outputs*l.batch*(l.steps-1);
    for (i = l.steps-1; i >= 0; --i) {
        if(i != 0){
			float * output_float = l.output_gpu;
			float * sum_float = output_float - l.outputs*l.batch;
			cl_mem sum = (cl_mem) sum_float;
			copy_ongpu(l.outputs*l.batch, sum/*l.output_gpu - l.outputs*l.batch*/, 1, l.prev_state_gpu, 1);
		}
		float * delta_float = (float *) l.delta_gpu;
        float *prev_delta_gpu = (i == 0) ? 0 : delta_float - l.outputs*l.batch;
       ...
       // l.output_gpu -= l.outputs*l.batch;
		...
		float * sum2 = delta_gpu_float - l.outputs*l.batch;
		l.delta_gpu = (cl_mem) sum2;
