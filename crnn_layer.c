...
void forward_crnn_layer_gpu(layer l, network net)
{
    ...
    for (i = 0; i < l.steps; ++i) {
        ...
        if(net.train){
			float * state_float = (float *) l.state_gpu;
			float * sum_float = state_float + l.hidden*l.batch;
			l.state_gpu = (cl_mem) sum_float;
...
void backward_crnn_layer_gpu(layer l, network net)
{
    ...
    increment_layer(&self_layer,   l.steps - 1);
    increment_layer(&output_layer, l.steps - 1);
	float * state_gpu_float = (float *) l.state_gpu;
	float * sum1 = state_gpu_float + l.hidden*l.batch*l.steps;
	l.state_gpu = (cl_mem) sum1;
   // l.state_gpu += l.hidden*l.batch*l.steps;
    for (i = l.steps-1; i >= 0; --i) {
        copy_ongpu(l.hidden * l.batch, input_layer.output_gpu, 1, l.state_gpu, 1);
        axpy_ongpu(l.hidden * l.batch, 1, self_layer.output_gpu, 1, l.state_gpu, 1);
		float * state_float = (float *) l.state_gpu;
        s.input_gpu = state_float;
		float * delta_float = (float *) self_layer.delta_gpu;
        s.delta_gpu = delta_float;
        backward_convolutional_layer_gpu(output_layer, s);
		float * state_gpu_float = (float *) l.state_gpu;
		float * sum = state_gpu_float - l.hidden*l.batch;
		l.state_gpu = (cl_mem) sum;
     //   l.state_gpu -= l.hidden*l.batch;

        s.input_gpu = l.state_gpu;
		float * delta_gpu1_float = (float *) self_layer.delta_gpu;
		s.delta_gpu = delta_gpu1_float - l.hidden*l.batch;
       ...
       // s.delta_gpu = self_layer.delta_gpu - l.hidden*l.batch;
        ...
        if (i > 0 && l.shortcut){
			float * delta_new_float = (float *) self_layer.delta_gpu;
			float *sum2_float = delta_new_float - l.hidden*l.batch;
			cl_mem sum2 = (cl_mem) sum2_float;
			axpy_ongpu(l.hidden*l.batch, 1, self_layer.delta_gpu, 1, sum2/*self_layer.delta_gpu - l.hidden*l.batch*/, 1);
		}
