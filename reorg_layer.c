...
void forward_reorg_layer_gpu(layer l, network net)
{
    ...
    if(l.flatten){
        ...
        for(i = 0; i < l.batch; ++i){
			float * sum_float = net.input_gpu + i*l.inputs;
			cl_mem sum = (cl_mem) sum_float;
			float * output_float = (float *) l.output_gpu;
			float * sum2_float = output_float + i*l.outputs;
			cl_mem sum2 = (cl_mem) sum2_float;

            copy_ongpu(l.inputs, sum/*net.input_gpu + i*l.inputs*/, 1, sum2/*l.output_gpu + i*l.outputs*/, 1);
        }
    } else if (l.reverse) {
...
void backward_reorg_layer_gpu(layer l, network net)
{
    if(l.flatten){
        ...
        int i;
        for(i = 0; i < l.batch; ++i){
			float * sum_float = net.delta_gpu + i*l.inputs;
			cl_mem sum = (cl_mem) sum_float;
			float * delta_float = (float *) l.delta_gpu;
			float * sum2_float = delta_float + i*l.outputs;
			cl_mem sum2 = (cl_mem) sum2_float;

            copy_ongpu(l.inputs, sum2/*l.delta_gpu + i*l.outputs*/, 1, sum/*net.delta_gpu + i*l.inputs*/, 1);
        }
    } else if(l.reverse){
